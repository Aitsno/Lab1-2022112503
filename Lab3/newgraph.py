...
import sys
import argparse
import secrets  # 替换 random 为 secrets
import os
from collections import defaultdict
from heapq import heappop, heappush
from math import log
import subprocess  # 用于安全执行系统命令

from graphviz import Digraph
class TextGraph:
    """文本图处理模块，提供文本图构建、桥接词查找、路径分析等功能"""
    def __init__(self):
    ...
    def generate_new_text(self, input_text):
        ...
        if not original_words:  # 新增空列表处理
            return ""  # 直接返回空字符串，避免后续索引操作
        ...
        for i in range(len(lower_words) - 1):
            ...
            if bridges:
                bridge = secrets.choice(bridges)  # 使用 secrets 替代 random
    ...
    def random_walk(self):
        ...
        current = secrets.choice(list(self.nodes))  # 使用 secrets 替代 random
        path = [current]
        edges_visited = set()
        total_weight = 0
        stop_flag = False
        ...
        while True:
            ...
            if next_nodes:
                next_node, weight = secrets.choice(next_nodes)  # 使用 secrets 替代 random
            else:
                break  # 无出边时退出
    ...
    def save_walk_result(self, path, edges, filename="random_walk.txt"):
        ...
        try:
            ...
            print("错误：命令执行失败")
        except Exception as e:  # 保留通用异常处理，但放在最后
            print(f"未知错误：{e}")

    def save_graph_image(self, filename="text_graph", image_format="png"):
        """
        使用Graphviz生成有向图图像并保存到文件
        
        参数:
            filename (str): 文件名(不带扩展名)
            image_format (str): 图像格式(png, pdf, svg等)
        ...
        if image_format.lower() not in valid_formats:
            print(f"错误：不支持的图像格式 '{image_format}'，支持的格式: {', '.join(valid_formats)}")
            return None
        try:
            # 尝试从环境变量获取Graphviz路径（避免硬编码）
            ...
            for from_node, targets in self.graph.items():
                for to_node, weight in targets.items():
                    dot.edge(from_node, to_node, label=str(weight), fontsize='10')

            # 设置字体（确保中文显示）
            dot.attr('node', shape='circle', fontname='SimHei')  # 使用支持中文的字体
            dot.attr('edge', fontname='SimHei', fontsize='10')

            # 渲染图形
            filepath = dot.render(filename=filename, format=image_format, cleanup=True)
            print(f"图形已成功保存到: {filepath}")
            return filepath
        except FileNotFoundError:
            print("错误：未找到Graphviz可执行文件，请检查环境变量配置")
            return None
        except subprocess.CalledProcessError:
            print("错误：Graphviz命令执行失败，请检查参数和文件权限")
            return None
        except Exception as e:  # 保留通用异常处理，但放在最后
            print(f"生成图形时发生未知错误: {e}")
            return None
    ...
    def show_shortest_path_on_graph(self, word1, word2=None):
        ...
        if word2 is None:
            ...
            for target, (path, dist) in results.items():
                ...
                self._highlight_path_on_graph(path, f"shortest_path_{word1}_to_{target}")
        else:
            if word2 not in self.nodes:
            ...
            if path:
                ...
                self._highlight_path_on_graph(path,f"shortest_path_{word1}_to_{word2}")
            else:
                print(f"提示: 从 {word1} 到 {word2} 不可达")

    def _highlight_path_on_graph(self, path, filename):
        """将最短路径标注在图上并展示"""
        try:
            os.environ["GRAPHVIZ_DOT"] = r"D://Graphviz-12.2.1-win64//bin//dot.exe"
            ...
            # 使用 subprocess 安全执行系统命令
            if sys.platform == 'win32':
                os.startfile(filepath)  # Windows 下路径可控，使用 os.startfile
            elif sys.platform == 'darwin':
                subprocess.run(['open', filepath], check=True)  # macOS 安全执行
            else:
                subprocess.run(['xdg-open', filepath], check=True)  # Linux 安全执行
    ...
    参数:
        damping_factor (float): 阻尼因子，默认0.85
        max_iterations (int): 最大迭代次数，默认100
        tolerance (float): 收敛阈值，默认1e-6
        text (str): 可选，用于计算初始权重的文本
    ...
    """
        ...
        for _ in range(max_iterations):
            ...
            for node in self.nodes:
                if out_degree[node] > 0:
                    new_pagerank[node] += damping_factor*zero_out_degree_pr/(num_nodes-
...
def command_line_main():
    ...
    负责解析命令行参数，读取输入文本，构建文本图，并提供交互菜单
    """
    ...
    while True:
        ...
        if choice == '1':
            ...
            image_format = fileext[1:] if fileext else 'png'
            saved_file = graph.save_graph_image(filename=filepath,image_format=image_format)
            if saved_file:
                print(f"提示: 有向图已保存到 {saved_file}")
            else:
                print("错误: 保存有向图失败")
